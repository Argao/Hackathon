...
public class SimulacaoOrchestratorTests
{
    ...
    [Fact]
    public async Task RealizarSimulacaoAsync_ComEventPublisherFalhando_DevePropagarExcecao()
    {
        // Arrange
        var command = new RealizarSimulacaoCommand(10000m, 12);
        var ct = CancellationToken.None;
        ...
        _mockEventPublisher
            ...
            .Throws(new Exception("Erro no EventPublisher"));

        // Act & Assert
        var action = () => _orchestrator.RealizarSimulacaoAsync(command, ct);
        await action.Should().ThrowAsync<Exception>()
            .WithMessage("Erro no EventPublisher");
    }
}
