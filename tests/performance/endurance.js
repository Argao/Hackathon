...
const TEST_TYPE = __ENV.TEST_TYPE || 'endurance';
const USERS = parseInt(__ENV.USERS) || 30;
const DURATION = parseInt(__ENV.DURATION) || 600; // 10 minutos
...
export const options = {
    stages: [
        ...
        { duration: `${DURATION}s`, target: USERS }, // Carga sustentada
...
export function setup() {
    console.log(`⏰ Iniciando teste de endurance`);
    console.log(`📊 Configurações:`);
    console.log(`   • URL Base: ${BASE_URL}`);
    console.log(`   • Usuários: ${USERS}`);
    console.log(`   • Duração: ${DURATION}s (${Math.round(DURATION/60)} minutos)`);
...
export default function(data) {
    ...
    // Para endurance, usar distribuição realista
    const endpoints = [
        { method: 'GET', path: '/health', name: 'Health Check', weight: 5 },
        { method: 'GET', path: '/simulacao', name: 'Listar Simulações', weight: 30 },
        ...
        { method: 'GET', path: `/telemetria/por-dia?dataReferencia=${dataAtual}`, name: 'Telemetria por Dia', weight: 10 },
    ];
    ...
    switch (selectedEndpoint.method) {
        case 'GET':
            ...
            break;
        case 'POST':
            // Dados de simulação realistas para endurance
    ...
    // Sleep para endurance (carga sustentada)
    sleep(0.5);
}
